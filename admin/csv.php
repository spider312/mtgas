...
$equery = query("SELECT * FROM extension WHERE `se` = '$ext' ; ", 'Extension selection', $card_connection) ;
if ( $e = mysql_fetch_object($equery) ) {
	header("Content-type: application/csv-tab-delimited-table"); 
	header("Content-disposition: attachment; filename=\"$ext.csv\"");
	$cquery = query('SELECT * FROM card_ext, card  WHERE `card_ext`.`ext` = '.$e->id.' AND `card`.`id` = `card_ext`.`card` ORDER BY `card`.`name`', 'Card selection', $card_connection) ;
	echo '"Name";"Color";"Cost";Converted;"Supertypes";"Types";"Subtypes"'."\n" ;
	while ( $c = mysql_fetch_object($cquery) ) {
		$j = json_decode($c->attrs) ;
		echo '"'.$c->name.'";"'.$j->color.'";"'.$c->cost.'";'.$j->converted_cost.';"' ;
		// Types, if exists
