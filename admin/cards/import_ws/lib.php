...
class ImportExtension {
	...
	function addcard($card_url, $rarity, $oldname, $cost, $types, $text, $url, $multiverseid='') {
		...
		if ( isset($this->handler) ) { // Send a copy of raw data to client
			$json = json_encode($card) ;
			$this->handler->broadcast($json) ;
		}
		$this->cards[] = $card ;
		return $card ;
	}
	...
	function import($ext='', $apply=false) {
		...
		if ( $ext == '' ) 
			...
			global $db_cards ;
			$ext = strtoupper($ext) ;
			$res = $db_cards->exists("SELECT * FROM extension WHERE `se` = '$ext' ; ") ; // First search in E
			if ( ! $res ) // Take another chance with sea
				$res = $db_cards->exists("SELECT * FROM extension WHERE `sea` = '$ext' ; ") ;	
			if ( ! $res ) {
				echo 'Extension ['.$ext.'] not found'."\n" ;
				if ( $apply ) {
					$ext_id = $db_cards->insert("INSERT INTO extension (`se`, `name`) VALUE
						 ('$ext', '".$db_cards->escape($this->name)."')") ;
					echo 'Created'."\n" ;
				}
			} else {
				$ext_id = $res->id ;
...
class ImportCard {
	...
	function addimage($url) { // For double faced cards
		...
		$this->images[] = $url ;
		// Can't nbimages++ here because not all addimage should do this (mainly transform)
		return true ;
	}
