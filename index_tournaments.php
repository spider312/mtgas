   <div id="tournaments" class="section"><!-- - - - - - - - - - - tournaments - - - - - - - - - - -->
    ...
    <form id="tournament_create" action="tournament/json/create.php" method="post">
     <label title="<?=__('index.tournaments.create.format.title');?>">
	  <?=__('index.tournaments.create.format');?>
     ...
     <label title="<?=__('index.tournaments.create.name.title');?>">
	  <?=__('index.tournaments.create.name');?>
      <input type="text" id="tournament_name" name="name" size="64"
       placeholder="<?=__('index.tournaments.create.name_placeholder');?>">
     </label>
     <label title="<?=__('index.tournaments.create.players.title');?>">
	  <?=__('index.tournaments.create.players');?>
      <input type="text" id="tournament_players" name="players" value="2" size="2" maxlength="2">
     </label>
     <div id="limited">
      <label id="tournament_suggestions_label"
       title="<?=__('index.tournaments.create.suggestions.title');?>">
	   <?=__('index.tournaments.create.suggestions');?>
       <select id="tournament_suggestions"></select>
      </label>
      <label id="tournament_boosters_label" title="<?=__('index.tournaments.create.boosters.title');?>">
	   <?=__('index.tournaments.create.boosters');?>
      ...
      <label id="booster_suggestions_label"
	   title="<?=__('index.tournaments.create.booster_add.title');?>">
	   <?=__('index.tournaments.create.booster_add');?>
     ...
     <fieldset id="tournament_options" class="shrinked"
	  title="<?=__('index.tournaments.create.options.title');?>">
      <legend>
	   <input type="button" id="tournament_options_toggle" value="+">
	   <?=__('index.tournaments.create.options');?>
	  </legend>
     ...
     <table id="running_tournament_list">
      <thead>
       <tr>
        ...
        <td title="<?=__('index.tournaments.time_left.title');?>">
		 <?=__('index.tournaments.time_left');?>
		</td>
        <td><?=__('index.tournaments.players');?></td>
       </tr>
      </thead>
